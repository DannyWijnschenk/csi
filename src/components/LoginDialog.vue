  ...
  export default {
    ...
    methods : {
      login() {  //login on the server
        var url = this.$store.getters.serverUrl+"/csi/token";
        fetch(url, {
          "method": "GET",			  
      ...
      forceRefresh(callBack, args) {
        this.refreshIsCalling = true;
              ...
              fetch(this.$store.getters.serverUrl+"/csi/v1/refresh", {
    ...
    getRights(right) {
       var url = this.$store.getters.serverUrl+"/csi/v1/accessrights";
