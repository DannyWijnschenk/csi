        ...
        <div class="container-fluid card">
          ...
          <div class="card-body">
            ...
            <div class="row mb-2">
              ...
              <div class="col-sm-4"><input type="text" class="form-control" v-model="form.code"></div>
            </div>
            <div class="row mb-2">
              <div class="col-sm-3">Value</div>
              <div class="col-sm-6"><input type="text" class="form-control" v-model="form.value"/></div>
            </div>
          </div>
  ...
  export default {
    ...
    methods: {
      ...
      saveDataCB() {
        ...
        var method = ""  
        if (this.form.id == '') {
            url = this.$store.getters.serverUrl+"/csi/parameter";
            method = "POST";
        } else {
            url = this.$store.getters.serverUrl+"/csi/parameter/" + this.form.id;
            method = "PUT";
        }
        var body = this.form
        fetch(url, {
      ...
      getDataCB() {
        var url = this.$store.getters.serverUrl+"/csi/parameter/"+this.form.id;
        fetch(url, {
          "headers" : { "Authorization": 'Bearer ' + this.$store.getters.serverAccessToken },
          "method": "GET"
        }).then(response => {
