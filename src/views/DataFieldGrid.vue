<template>
    ...
    <div class="ps-2 pe-2">  <!-- padding start and padding end of 2 pixels -->
      <div class="container-fluid card ps-2" v-if='this.$store.getters.isLoggedIn'>  <!-- inner padding of 2 pixels -->
        ...
        <div class="card-body">
          <div class="row">
            <grid-data ref="grid" titleHeading="DataField" table="DataField" v-on:gridClickRow="gridclickrow"></grid-data>
          </div>
        </div>
      </div>
    </div>  <!--card -->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      editParameter(id) {
        this.$router.push('/datafield/'+id);
      },
      addParameter() {
        this.$router.push('/datafield/new');
      },
