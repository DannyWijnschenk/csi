  ...
  export default {
    ...
    methods: {
      ...
      saveDataCB() {
        ...
        var method = ""  
        if (this.form.id == '') {
            url = this.$store.getters.serverUrl+"/csi/csidomain";
            method = "POST";
        } else {
            url = this.$store.getters.serverUrl+"/csi/csidomain/" + this.form.id;
            method = "PUT";
        }
        var body = this.form
        fetch(url, {
      ...
      getDataCB() {
        var url = this.$store.getters.serverUrl+"/csi/csidomain/"+this.form.id;
        fetch(url, {
          "headers" : { "Authorization": 'Bearer ' + this.$store.getters.serverAccessToken },
          "method": "GET"
        }).then(response => {
