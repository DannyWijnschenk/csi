  ...
  export default {
    ...
    methods: {
      ...
      saveDataCB() {
        ...
        var method = ""  
        if (this.form.id == '') {
            url = this.$store.getters.serverUrl+"/csi/useredit/" + this.form.initials;
            method = "POST";
        } else {
            url = this.$store.getters.serverUrl+"/csi/useredit/" + this.form.initials;
            method = "POST";
      ...
      getDataCB() {
        var url = this.$store.getters.serverUrl+"/csi/useredit/"+this.form.id;
        fetch(url, {
          "headers" : { "Authorization": 'Bearer ' + this.$store.getters.serverAccessToken },
          "method": "GET"
        }).then(response => {
      ...
      fetchDomains() {
        const url = this.$store.getters.serverUrl+"/csi/domain";
    ...
    fetchDomainFields() {
        const fieldRequests = this.domainList.map(domain => {
            const fieldUrl = this.$store.getters.serverUrl+"/csi/domain/" + domain.id + "/field";
    ...
    fetchRole() {
        const url = this.$store.getters.serverUrl+"/csi/rolelist";
