<template>
  ...
  <div class="ps-2 pe-2"> <!-- padding start and padding end of 2 pixels -->
    <div class="container-fluid card ps-2" v-if='this.$store.getters.isLoggedIn'> <!-- inner padding of 2 pixels -->
      ...
      <div class="card-body">
        <div class="row">
          <grid-data ref="grid" titleHeading="SourceFormGroup" table="SourceFormGroup"
            v-on:gridClickRow="gridclickrow"></grid-data>
        </div>
      </div>
    </div>
  </div> <!--card -->
  ...
  methods: {
    ...
    async exportExcel() {
      try {
        ...
        /** ---------------------------
         *  SHEET 2: this.groupedForms
         *  --------------------------*/
        const sheet2Data = [
          ["SourceFromBlock", "SourceForm", "Size", "Count", "BlockTotalSize", "BlockTotalCount"],
        ]
        ...
        // groupedForms: { AFE: [{sourceForm,totalSize,totalCount,...}], ... }
        const entries2 = Object.entries(this.groupedForms || {}).sort(([a], [b]) => a.localeCompare(b))

        for (const [prefix, itemsRaw] of entries2) {
          const items = (itemsRaw || []).slice().sort((a, b) => (a.sourceForm || "").localeCompare(b.sourceForm || ""))

          if (items.length === 0) continue

          const blockStartRow = r2

          // block totals (sum of per-form)
          const blockTotalSize = items.reduce((sum, sf) => sum + (Number(sf.totalSize) || 0), 0)
          const blockTotalCount = items.reduce((sum, sf) => sum + (Number(sf.totalCount) || 0), 0)

          for (const sf of items) {
            const form = sf.sourceForm || ""

            // per-form size/count
            const formSizeGb = Number(((Number(sf.totalSize) || 0) / 1024).toFixed(2)) + " Gb"
            const formCount = (Number(sf.totalCount) || 0).toLocaleString("uk-UA")

            sheet2Data.push(["", form, formSizeGb, formCount, "", ""])
            r2++
          }

          const blockEndRow = r2 - 1

          // write prefix in first row of block and merge down
          ...
          // merged BlockTotal columns (E,F) for the whole block
          merges2.push({
            s: { r: blockStartRow, c: 4 },
            e: { r: blockEndRow, c: 4 },
          })
          merges2.push({
            s: { r: blockStartRow, c: 5 },
            e: { r: blockEndRow, c: 5 },
          })

          sheet2Data[blockStartRow][4] = Number((blockTotalSize / 1024).toFixed(2)) + " Gb"
          sheet2Data[blockStartRow][5] = blockTotalCount.toLocaleString("uk-UA")
        ...
        borderBlocks2.forEach(({ start, end }) => {
          for (let r = start; r <= end; r++) {
            for (let c = 0; c <= 5; c++) {
